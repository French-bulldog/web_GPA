...
let allSelects = document.querySelectorAll("select"); //靜態NodeList，之後新增在內的標籤並不會被加進來
...
addButton.addEventListener("click", () => {
    ...
    newInput1.placeholder = "選修課";
    newInput2.placeholder = "班級編號";
    newInput3.placeholder = "學分";
...
function handleSorting(direction) {
    ...
    for (let i = 0; i < graders.length; i++) {
        let class_name = graders[i].children[0].value; // 學科
        let class_number = graders[i].children[1].value; // 班級編號
        let class_credit = graders[i].children[2].value; // 學分
        let class_grade = graders[i].children[3].value; // 成績
